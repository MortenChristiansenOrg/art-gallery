...
## Migration: Fix Auth Token Validation

Current issue: Token validation runs every render.

```tsx
// BAD - validates every render
function AuthProvider({ children }) {
  const { isAuthenticated } = useConvexAuth();

  // This runs on EVERY render
  if (isAuthenticated && !validateToken(getToken())) {
    logout();
  }
...
// GOOD - cache validation, run once on auth change
function AuthProvider({ children }) {
  const { isAuthenticated } = useConvexAuth();
